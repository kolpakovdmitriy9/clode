 ...
 *   6 … 7.7  — продолжает подниматься, тускнеет и исчезает.
...
(() => {
  ...
  const PATH = [
    ...
    [7,    -1.13,  0.33,  1.10,  0.45, 0.55],
    [7.7,  -1.22,  0.31,  1.15,  0,    0.35],
  ];
  ...
  // последней — хвост докатывается и растворяется (~1.6 с).
  const TIMELINE = [
    [0.00, 0], [0.27, 1], [0.60, 2], [0.87, 3], [1.03, 4], [1.13, 5],
    [1.24, 6], [1.42, 7], [1.53, 8], [1.67, 9], [1.83, 10], [1.97, 11],
    [2.07, 12], [2.20, 13], [2.80, 16.5], [3.40, 19], [3.80, 20.8],
  ];
  const RUN_END = TIMELINE[TIMELINE.length - 1][0];
  ...
  function run() {
    ...
    setTimeout(() => {
      ...
      const tick = (now) => {
        const t = (now - t0) / 1000;
        render(playhead(t));
        if (t < RUN_END) requestAnimationFrame(tick);
        else finish();
      };
